...
class MarkdownCompatibilityHandler {
		...
		return [
			{
				...
				detector: (content: string) =>
					/\[\[(?!\!)[^\]]+\]\]/.test(content),
				converter: (content: string, options?: ConversionOptions) =>
					this.convertWikilinks(content, options),
			},
			{
				...
				converter: (content: string, options?: ConversionOptions) =>
					this.convertImageWikilinks(content, options),
			},
			{
				name: "Obsidian Tags",
				...
				detector: (content: string) =>
					/#[\w\-\/]+(?:\s|$|[^\w\-\/])/.test(content),
				converter: (content: string, options?: ConversionOptions) =>
					this.convertTags(content, options),
			},
			{
				...
				converter: (content: string, options?: ConversionOptions) =>
					this.convertCallouts(content, options),
			},
			{
				...
				detector: (content: string) =>
					/\$\$[\s\S]*?\$\$|\$[^$\n]+\$/.test(content),
				converter: (content: string, options?: ConversionOptions) =>
					this.convertMath(content, options),
			},
			{
				name: "Plugin Content",
				description: "Mermaid, Dataview, and other plugin syntax",
				detector: (content: string) =>
					/```(?:mermaid|dataview|ad-|query)/.test(content) ||
					/%%[\s\S]*?%%/.test(content),
				converter: (content: string, options?: ConversionOptions) =>
					this.convertPluginContent(content, options),
			},
		];
	}
	...
	analyzeContent(content: string): CompatibilityReport {
		...
		const compatibilityScore =
			this.calculateCompatibilityScore(detectedVariants);
	...
	convertContent(
		...
		for (const variant of this.variants) {
			if (variant.detector(result.convertedContent)) {
				const conversionResult = variant.converter(
				...
				result.removedElements.push(
					...conversionResult.removedElements,
				);
				result.changedElements.push(
					...conversionResult.changedElements,
				);
			}
		}
	...
	private calculateCompatibilityScore(detectedVariants: string[]): number {
		...
		return Math.max(
			0,
			Math.round(
				((totalVariants - incompatibleVariants) / totalVariants) * 100,
			),
		);
	}
	...
	private generateRecommendations(detectedVariants: string[]): string[] {
		...
		if (detectedVariants.includes("Obsidian Wikilinks")) {
			recommendations.push(
				"Convert wikilinks to standard Markdown links for better compatibility",
			);
		}
		if (detectedVariants.includes("Obsidian Callouts")) {
			recommendations.push(
				"Convert callouts to standard blockquotes with emoji indicators",
			);
		}
		if (detectedVariants.includes("Math Expressions")) {
			recommendations.push(
				"Consider converting math expressions to code blocks or images",
			);
		}
		if (detectedVariants.includes("Plugin Content")) {
			recommendations.push(
				"Plugin-specific content may not render correctly in GitHub Gist",
			);
		}
	...
	private convertWikilinks(
		...
		if (!options?.preserveWikilinks) {
			...
			if (
				result.changedElements.length > 0 ||
				result.removedElements.length > 0
			) {
				if (options?.compatibilityMode === "github-native") {
					result.warnings.push(
						`Converted ${result.changedElements.length} wikilinks to standard markdown links`,
					);
				} else {
					result.warnings.push(
						`Converted ${result.changedElements.length} wikilinks for compatibility`,
					);
				}
			}
		}
	...
	private convertTags(
		...
		if (options?.convertTags) {
			...
			if (options?.compatibilityMode === "github-native") {
				result.warnings.push(
					"Tags preserved in original format for GitHub Gist compatibility",
				);
				return result;
			}
			...
			while ((match = tagRegex.exec(content)) !== null) {
				...
				const beforeTag = content.substring(
					lineStart,
					match.index + match[0].indexOf(originalText),
				);
				...
				if (options?.compatibilityMode === "strict") {
					...
					const format =
						options?.tagConversionFormat || "inline-code";
			...
			if (
				...
				result.warnings.push(
					`Processed ${result.changedElements.length + result.removedElements.length} tags using ${options?.tagConversionFormat || "inline-code"} format`,
				);
			}
		}
	...
	private convertCallouts(
		...
		if (options?.convertCallouts) {
			...
			if (result.changedElements.length > 0) {
				result.warnings.push(
					`Converted ${result.changedElements.length} callouts to blockquotes with emoji indicators`,
				);
			}
		}
	...
	private convertMath(
		...
		while ((match = blockMathRegex.exec(content)) !== null) {
			...
			if (options?.handleMath === "remove") {
				...
				result.removedElements.push(
					`Math block: ${originalText.substring(0, 50)}...`,
				);
		...
		while (
			(match = inlineMathRegex.exec(result.convertedContent)) !== null
		) {
			const originalText = match[0];
			const mathContent = match[1].trim();
		...
		const totalMathElements =
			result.changedElements.length + result.removedElements.length;
		if (totalMathElements > 0) {
			// Update warning message since GitHub Gist has native math support since 2022
			if (options?.compatibilityMode === "github-native") {
				result.warnings.push(
					`Mathematical expressions preserved for GitHub native rendering`,
				);
			} else {
				result.warnings.push(
					`Processed ${totalMathElements} mathematical expressions - they may not render correctly in older platforms`,
				);
			}
		}
	...
	private convertPluginContent(
		...
		const improperMermaidRegex =
			/```\s*\n((?:flowchart|graph)[\s\S]*?)\n```/g;
		let improperMatch;
		let mermaidFixCount = 0;

		while (
			(improperMatch = improperMermaidRegex.exec(
				result.convertedContent,
			)) !== null
		) {
		...
		if (mermaidFixCount > 0) {
			result.warnings.push(
				`Fixed ${mermaidFixCount} improperly tagged Mermaid diagrams for GitHub native rendering`,
			);
		}
		...
		if (options?.compatibilityMode !== "github-native") {
			...
			result.warnings.push(
				"Mermaid diagrams preserved for GitHub native rendering",
			);
		}
		...
		while (
			(dataviewMatch = dataviewRegex.exec(result.convertedContent)) !==
			null
		) {
			...
			if (options?.handlePluginContent === "remove") {
				...
				const format =
					options?.dataviewConversionFormat || "descriptive-blocks";
				let convertedText: string;
		...
		while (
			(commentMatch = commentRegex.exec(result.convertedContent)) !== null
		) {
			...
			if (options?.compatibilityMode === "github-native") {
				...
				const format =
					options?.commentConversionFormat || "html-comments";
				let convertedText: string;
		...
		while (
			(admonitionMatch = admonitionRegex.exec(
				result.convertedContent,
			)) !== null
		) {
		...
		const totalPluginElements =
			result.changedElements.length + result.removedElements.length;
		if (totalPluginElements > 0) {
			result.warnings.push(
				`Processed ${totalPluginElements} plugin-specific elements that may not render correctly in GitHub Gist`,
			);
		}
...
export default class QuickShareNotePlugin extends Plugin {
	...
	async onload() {
		try {
			...
			this.addCommand({
				id: "copy-title-and-link",
				name: "Copy title and link",
				callback: () => this.copyCurrentNoteGistLink(),
			});
		} catch (error) {
	...
	async publishNoteToGist() {
		...
		let progressNotice = new Notice(
			`Preparing to publish: ${activeFile.name}...`,
			0,
		);
		...
		try {
			...
			progressNotice.setMessage(
				`Processing images in: ${activeFile.name}...`,
			);
			const updatedContent = await this.uploadImagesAndReplaceLinks(
				fileContent,
				(current, total) => {
					progressNotice.setMessage(
						`Uploading images: ${current}/${total} - ${activeFile.name}`,
					);
				},
			);

			const gistIdMatch = updatedContent.match(
				/gist-publish-url: https.*\/(.*)/,
			);

			const contentToPublish = this.prepareContentForPublishing(
				updatedContent,
				activeFile.name,
			);

			const gistUrl = await this.publishToGist(
				gistIdMatch,
				activeFile.name,
				contentToPublish,
				progressNotice,
	...
	async analyzeCurrentNoteCompatibility() {
		...
		try {
			...
			const report = this.compatibilityHandler.analyzeContent(
				parsed.content,
			);
			...
			if (report.detectedVariants.length === 0) {
				reportLines.push(
					"✅ **No compatibility issues found!** Your note uses standard Markdown that will render perfectly in GitHub Gist.",
				);
			} else {
				reportLines.push(
					"The following Obsidian-specific elements were detected:",
				);
				reportLines.push("");
			...
			reportLines.push("---");
			reportLines.push(
				"*This report was generated by the Quick Share Note to Gist plugin*",
			);
			...
			new Notice(
				`Compatibility analysis complete! Score: ${report.compatibilityScore}/100\nDetailed report copied to clipboard.`,
				5000,
			);
		} catch (error) {
	 ...
	 */
	private prepareContentForPublishing(
		content: string,
		fileName: string,
	): string {
		...
		const conversionResult = this.compatibilityHandler.convertContent(
			parsed.content,
			conversionOptions,
		);
		...
		if (
			this.settings.showCompatibilityReport &&
			...
			if (
				conversionResult.changedElements.length > 0 ||
				conversionResult.removedElements.length > 0
			) {
				const totalChanges =
					conversionResult.changedElements.length +
					conversionResult.removedElements.length;
				new Notice(
					`Applied ${totalChanges} compatibility conversions for GitHub Gist`,
					3000,
				);
			}
		}
		...
		const frontmatter =
			this.settings.showFrontmatter && parsed.hasFrontmatter
				? parsed.frontmatter
				: "";
	 ...
	 */
	private async publishToGist(
		gistIdMatch: RegExpMatchArray | null,
		fileName: string,
		content: string,
	 ...
	 */
	private async finalizePublishing(
		file: TFile,
		gistUrl: string,
		progressNotice: Notice,
	): Promise<void> {
	...
	async createNewGist(activeFileName: string, contentToPublish: string) {
		try {
			...
			throw new Error(
				`Failed to create gist: ${this.getErrorMessage(error)}`,
			);
		}
	}
	...
	async updateGist(
		...
		try {
			...
			throw new Error(
				`Failed to update gist: ${this.getErrorMessage(error)}`,
			);
		}
	}
	...
	async updateFrontmatter(file: TFile, url: string) {
		try {
			...
			if (parsed.hasFrontmatter) {
				const existingMatch = parsed.frontmatter.match(
					/gist-publish-url:\s*(.+)/,
				);
			...
			if (parsed.hasFrontmatter) {
				...
				const gistUrlIndex = contentLines.findIndex((line) =>
					line.startsWith("gist-publish-url:"),
				);
			...
			throw new Error(
				`Failed to update file frontmatter: ${this.getErrorMessage(error)}`,
			);
		}
	}
	...
	async uploadImagesAndReplaceLinks(
		...
		for (let i = 0; i < matches.length; i++) {
			...
			try {
				const attachFile = this.app.metadataCache.getFirstLinkpathDest(
					match[1],
					notePath,
				);
				if (attachFile == null) {
	...
	async uploadImageToImgur(imageData: ArrayBuffer): Promise<string> {
		try {
			...
			throw new Error(
				`Failed to upload image to Imgur: ${this.getErrorMessage(error)}`,
			);
		}
	}
	...
	async copyCurrentNoteGistLink() {
		...
		try {
			const fileContent = await this.app.vault.read(activeFile);
			const gistUrlMatch = fileContent.match(
				/gist-publish-url: https.*\/(.*)/,
			);

			if (!gistUrlMatch) {
				new Notice(
					"No gist URL found for this note. Please publish it first.",
				);
				return;
			}

			const gistUrl = gistUrlMatch[0].replace("gist-publish-url: ", "");
			const noteTitle = activeFile.basename;

			this.copyLinkToClipboard(gistUrl, noteTitle);
		} catch (error) {
			console.error("Failed to copy gist link:", error);
			new Notice("Failed to copy gist link");
		}
	}
	...
	private validateSettings(): boolean {
		if (!this.settings.githubToken.trim()) {
			new Notice(
				"GitHub token is required. Please configure it in plugin settings.",
			);
			return false;
		}
		...
		if (!this.settings.imgurClientId.trim()) {
			new Notice(
				"Imgur client ID is required. Please configure it in plugin settings.",
			);
			return false;
		}
		...
		if (
			!this.settings.githubToken.match(/^github_pat_[A-Za-z0-9_]+$/) &&
			!this.settings.githubToken.match(/^gh[a-z]_[A-Za-z0-9_]+$/) &&
			!this.settings.githubToken.match(/^[a-f0-9]{40}$/)
		) {
			new Notice(
				"GitHub token appears to be invalid. Please check your token format.",
			);
			return false;
		}
	...
	private handlePublishError(error: any) {
		...
		if (
			...
			new Notice(
				"GitHub API access denied. Check your token permissions or rate limits.",
			);
			...
			new Notice(
				"Image upload failed. Please check your Imgur client ID.",
			);
		} else {
		...
		this.registerEvent(
			this.app.vault.on("modify", async (file) => {
				...
				if (
					...
					(await this.shouldAutoSync(file))
				) {
	...
	private async hasGistUrl(file: TFile): Promise<boolean> {
		try {
			...
			return (
				...
				/gist-publish-url:\s*https/.test(parsed.frontmatter)
			);
		} catch (error) {
			return false;
		}
	...
	private async autoSyncToGist(file: TFile) {
		...
		try {
			...
			const gistMatch = parsed.frontmatter.match(
				/gist-publish-url:\s*https:\/\/gist\.github\.com\/.*\/(.+)/,
			);
			if (!gistMatch) {
			...
			const contentWithImages =
				await this.uploadImagesAndReplaceLinks(content);
			const preparedContent = this.prepareContentForPublishing(
				contentWithImages,
				file.name,
			);

			// Update gist
			const response = await this.updateGist(
				gistMatch[1],
				file.name,
				preparedContent,
			);
			...
			if (this.settings.syncNotifications !== "none") {
				new Notice(
					`❌ Auto-sync failed: ${this.getErrorMessage(error)}`,
					4000,
				);
			}
		}
		...
		try {
			...
			if (this.settings.confirmPullOverride) {
				...
				const confirmed = window.confirm(
					"This will override your current note content with the Gist content. Continue?",
				);
				if (!confirmed) {
					pullNotice.hide();
					return;
			...
			const newContent = this.preserveFrontmatterForPull(
				parsed,
				firstFile.content,
			);
	 ...
	 */
	private preserveFrontmatterForPull(
		...
		const lastPulledIndex = frontmatterLines.findIndex((line) =>
			line.startsWith("last-pulled:"),
		);

		if (lastPulledIndex >= 0) {
			frontmatterLines[lastPulledIndex] =
	...
	private checkRateLimitEmergency() {
		...
		if (rateLimit.remaining <= this.settings.emergencyDisableThreshold) {
			...
			new Notice(
				`⚠️ Rate limit low (${rateLimit.remaining}/${rateLimit.limit}). Auto-sync disabled.`,
				5000,
			);
...
class QuickShareNoteSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Enter your GitHub personal access token with gist scope (supports all token formats)",
			)
			.addText((text) => {
				text.setPlaceholder(
					"github_pat_xxx, ghp_xxx, or classic 40-char token",
				)
					...
					.onChange(async (value) => {
						...
						text.inputEl.style.borderColor = isValid
							? ""
							: "#ff6b6b";
					});
				return text;
		...
		new Setting(containerEl)
			...
			.setDesc(
				"GitHub Native preserves GitHub Gist supported features, Permissive converts when possible, Strict removes all incompatible elements",
			)
			.addDropdown((dropdown) =>
				dropdown
					.addOption(
						"github-native",
						"GitHub Native (Preserve Gist features)",
					)
					.addOption(
						"permissive",
						"Permissive (Convert when possible)",
					)
					.addOption(
						"strict",
					...
					.onChange(
						async (
							value: "github-native" | "permissive" | "strict",
						) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Convert [[wikilinks]] to standard markdown links in GitHub Native mode",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Convert #tags to preserve data (GitHub Native mode ignores this setting)",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Choose how #tags are converted when not in GitHub Native mode",
			)
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(
						async (
							value: "inline-code" | "bold-labels" | "plain-text",
						) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Convert > [!note] callouts to blockquotes - GitHub Gist has native alert support since Nov 2023",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"How to handle LaTeX math expressions ($$ or $) - GitHub Gist has native support",
			)
			.addDropdown((dropdown) =>
				dropdown
		...
		new Setting(containerEl)
			...
			.setDesc(
				"How to handle Mermaid, Dataview, and other plugin-specific content (GitHub Native mode preserves Mermaid)",
			)
			.addDropdown((dropdown) =>
				dropdown
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Choose how %%comments%% are converted (GitHub Native mode preserves as HTML comments)",
			)
			.addDropdown((dropdown) =>
				dropdown
					.addOption(
					...
					.onChange(
						async (value: "html-comments" | "italic-text") => {
							this.plugin.settings.commentConversionFormat =
								value;
							await this.plugin.saveSettings();
						},
					),
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Choose how Dataview queries are converted when not removed",
			)
			.addDropdown((dropdown) =>
				dropdown
					.addOption(
						"descriptive-blocks",
						"Descriptive blocks with explanation",
					)
					.addOption("simple-blocks", "Simple labeled code blocks")
					.setValue(this.plugin.settings.dataviewConversionFormat)
					.onChange(
						async (
							value: "descriptive-blocks" | "simple-blocks",
						) => {
							this.plugin.settings.dataviewConversionFormat =
								value;
							await this.plugin.saveSettings();
						},
					),
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Show notifications about compatibility conversions during publishing",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Analyze the current note for Markdown compatibility issues",
			)
			.addButton((button) =>
				button
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Automatically sync note changes to GitHub Gist after editing stops",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"How long to wait after editing stops before syncing (prevents too frequent API calls)",
			)
			.addDropdown((dropdown) =>
				dropdown
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Automatically adjust sync delay based on GitHub API rate limit usage",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Add command to manually update Obsidian note from GitHub Gist",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Show confirmation dialog before overriding Obsidian content with Gist content",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value) => {
						this.plugin.settings.syncNotifications = value as
							| "all"
							| "errors"
							| "none";
						await this.plugin.saveSettings();
		...
		new Setting(containerEl)
			...
			.addText((text) => {
				text.setValue(this.plugin.getRateLimitDisplay()).setDisabled(
					true,
				);
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Manually update the current note with content from GitHub Gist",
			)
			.addButton((button) =>
				button
